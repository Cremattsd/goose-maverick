...
def score_leads(token, user_id):
    try:
        # Fetch recent contacts and their events
        status, contacts = realnex_get(f"Contacts?$filter=userId eq {user_id}&$top=50", token, is_odata=True)
        if status != 200:
            return

        status, events = realnex_get(f"Events?$filter=userId eq {user_id}&$top=50", token, is_odata=True)
        if status != 200:
            return

        # Prepare data for GPT-4o scoring
        contact_data = []
...
def poll_events(token, user_id, email, priority_filter, alarm_filter, due_date_days):
    ...
    while EVENT_POLLING_ENABLED.get(user_id, False):
        try:
            status, events = realnex_get(f"Events?$filter=userId eq {user_id}", token, is_odata=True)
            if status != 200:
                time.sleep(300)
                continue
            ...
            # Score leads periodically
            score_leads(token, user_id)
...
@app.route('/mailchimp-info', methods=['GET'])
def mailchimp_info():
    try:
        token = request.headers.get("Authorization", "").replace("Bearer ", "")
        if not token:
        ...
        status, groups = realnex_get("Groups?$top=10", token, is_odata=True)
        if status != 200:
            return jsonify({"error": "Failed to fetch RealNex groups"}), 500

        # Fetch Mailchimp audiences
        cursor.execute("SELECT mailchimp_api_key FROM user_settings WHERE user_id = ?", (user_id,))
        row = cursor.fetchone()
        mailchimp_api_key = row[0] if row and row[0] else MAILCHIMP_API_KEY
        if not mailchimp_api_key:
            return jsonify({"error": "Mailchimp API key not set. Configure it in Goose mode!"}), 400
        ...
        return jsonify({
            "realnex_groups": [{"id": g['crm_group_key'], "name": g['name']} for g in groups],
            "mailchimp_audiences": [{"id": a['id'], "name": a['name']} for a in audiences]
        })
    except Exception as e:
        logging.error(f"Mailchimp info error: {str(e)}")
        return jsonify({"error": f"Turbulence: {str(e)}"}), 500
...
def sync_mailchimp():
    try:
        ...
        if not token:
            return jsonify({"error": "No token, Maverick! Lock in! 🔒"}), 401
        if not group_id or not audience_id:
            return jsonify({"error": "Missing group or audience ID!"}), 400

        user_id = get_user_id(token)

        # Save Mailchimp API key if provided
        if mailchimp_api_key:
            cursor.execute('''INSERT OR REPLACE INTO user_settings 
                              (user_id, mailchimp_api_key) 
                              VALUES (?, ?)''',
                           (user_id, mailchimp_api_key))
            conn.commit()
        else:
            ...
            mailchimp_api_key = row[0] if row and row[0] else MAILCHIMP_API_KEY
            if not mailchimp_api_key:
                return jsonify({"error": "Mailchimp API key not set!"}), 400

        # Fetch contacts in the group
        status, group_contacts = realnex_get(f"GroupContacts?$filter=crm_group_key eq '{group_id}'", token, is_odata=True)
        if status != 200:
            return jsonify({"error": "Failed to fetch group contacts"}), 500

        contact_ids = [gc['crm_contact_key'] for gc in group_contacts]
        if not contact_ids:
            return jsonify({"message": "No contacts in this group to sync!"}), 200

        # Fetch contact details
        contacts = []
        for contact_id in contact_ids:
            status, contact = realnex_get(f"Contacts?$filter=crm_contact_key eq '{contact_id}'", token, is_odata=True)
            if status == 200 and contact:
        ...
        # Notify via WebSocket
        socketio.emit('notification', {'message': f"Synced {synced} contacts to Mailchimp! 🚀"}, namespace='/chat')

        return jsonify({"message": f"Synced {synced} contacts to Mailchimp!"})
    except Exception as e:
...
- *New Features*: 
  ...
  - Sync RealNex groups to Mailchimp audiences to kickstart campaigns!
- *Usage*:
  ...
  8. Sync a RealNex group to Mailchimp in Goose mode to start campaigns!
